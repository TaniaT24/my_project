...
class want_toRead extends StatefulWidget {
  const want_toRead({Key? key}) : super(key: key);

  @override
  _want_toRead createState() => _want_toRead();
}

class _want_toRead extends State<want_toRead> {
  ...
  void initState() {
   ...
   String dropdownValue='WantToRead';
      ...
      items: items.map<DropdownMenuItem<String>>((String items) {
       ...
       { ref.child(newvalue!).push().child('Book AName').set(snapshot.value['Book WName'] );
        _booksRef.child(snapshot.key.toString()).remove();
                          ...
                          title: new Text(snapshot.value['Book WName'] ,style: TextStyle(color:Colors.black,backgroundColor: Color.fromARGB(196, 249, 219, 219), fontStyle: FontStyle.italic,fontSize: 20,fontWeight:FontWeight.bold),),
                        );
                      })),],
                      ),),),
           ],
